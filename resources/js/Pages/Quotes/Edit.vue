<template>
  <AppLayout
    ...
    :breadcrumbs="[
      { label: 'Cotizaciones', href: route('quotes.index') },
      { label: quote.quote_number, href: route('quotes.show', quote.id) },
      { label: 'Editar' }
    ]"
  >
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input type="date" v-model="form.valid_until" :class="inputClass"/>
            </div>
          </div>
        </div>
        ...
        <div class="rounded-xl bg-white shadow-sm ring-1 ring-gray-100 overflow-hidden">
          <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h2 class="text-sm font-semibold text-gray-900">Ítems</h2>
            ...
            <table class="w-full">
              <thead>
                <tr class="bg-gray-50 text-left text-xs font-semibold uppercase tracking-wider text-gray-400">
                  <th class="px-4 py-3" style="width:38%;">Descripción</th>
                  <th class="px-4 py-3" style="width:25%;">Detalle</th>
                  <th class="px-4 py-3 text-center" style="width:9%;">Cant.</th>
                  <th class="px-4 py-3 text-right" style="width:15%;">Precio Unit.</th>
                  <th class="px-4 py-3 text-right" style="width:11%;">Subtotal</th>
        ...
        <div class="rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
          ...
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1.5">Notas para el cliente</label>
              <textarea v-model="form.notes" rows="3" :class="inputClass + ' resize-none'"/>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1.5">Términos y condiciones</label>
...
const form = useForm({
  ...
  tax_included: props.quote.tax_included,
